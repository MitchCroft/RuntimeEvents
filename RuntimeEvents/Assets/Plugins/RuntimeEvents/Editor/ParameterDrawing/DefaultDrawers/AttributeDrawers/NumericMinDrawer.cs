...
namespace RuntimeEvents.ParameterProcessors {
    /// <summary>
    /// Provide the functionality required to display minimum bounded numerical options within the Runtime Event inspector
    /// </summary>
    [CustomParameterDrawer(typeof(NumericMinAttribute))]
    public sealed class NumericMinDrawer : AErrorReportingDrawer {
        /*----------Functions----------*/
        //PUBLIC
        ...
        public override bool DisplayParameterValue(Rect position, PersistentParameterCache[] parameterCaches, GUIContent label) {
            ...
            NumericMinAttribute att = Attribute as NumericMinAttribute;
            ...
            if (currentObj is float) {
                ...
                using (GUIMixer.PushSegment(isDifferent)) {
                    ...
                    float newVal = Mathf.Min(EditorGUI.FloatField(position, label, val), att.Min);
            ...
            //Process the value as an integer
                ...
                using (GUIMixer.PushSegment(isDifferent)) {
                    ...
                    //If this type has a flags attribute, show a masking field
                    int newVal = Mathf.Min(EditorGUI.IntField(position, label, val), (int)att.Min);
