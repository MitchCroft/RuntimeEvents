...
namespace RuntimeEvents.ParameterProcessors {
    /// <summary>
    /// Provide the functionality required to display float options within the Runtime Event inspector
    /// </summary>
    [CustomParameterDrawer(typeof(float))]
    public sealed class FloatDrawer : AParameterDrawer {
        /*----------Functions----------*/
        //PUBLIC

        /// <summary>
        /// Float drawing is a simple process, only requires a single line
        /// </summary>
        ...
        /// <summary>
        /// Display the current float value setting to the inspector
        /// </summary>
        ...
        public override bool DisplayParameterValue(Rect position, PersistentParameterCache[] parameterCaches, GUIContent label) {
            ...
            float val = (float)parameterCaches[0].Value;
            ...
            using (GUIMixer.PushSegment(isDifferent)) {
                ...
                //Display a float field for modifying the value
                float newVal = EditorGUI.FloatField(position, label, val);
